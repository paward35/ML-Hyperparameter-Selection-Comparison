# ML-Hyperparameter-Selection-Comparison
This code relates to a short research paper that I completed with two classmates comparing several hyperparameter selection algorithms, along with the code generated to compare the algorithms. This project was part of a university machine learning course for which I received the highest possible grade (First Class Honours). 
